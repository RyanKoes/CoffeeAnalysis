...
def evaluate_training_model(model, X_tensor, y_mean, y_std):

    # Evaluation on training data
    model.eval()
    with torch.no_grad():
        predictions_normalized = model(X_tensor.to(device)).cpu()

    # Inverse transform predictions to original scale
    predictions_original_scale = predictions_normalized.numpy() * (y_std + 1e-8) + y_mean
    
    target_names = ['HPLC_Caff', 'HPLC_CGA', 'TDS']
    targets_original_scale = df[target_names].values
    ...
    ], predictions_original_scale, targets_original_scale
...
if __name__ == "__main__":
    ...
    model = CoffeeNet1024x5raw(1901).to(device)
    ...
    y = df[['HPLC_Caff', 'HPLC_CGA', 'TDS']].values
    ...
    predictions_original_scale = evaluate_training_model(model, df)


    # Plotting
    fig, axes = plt.subplots(1, 3, figsize=(18, 5))
    for i, name in enumerate(target_names):
        axes[i].scatter(targets_original_scale[:, i], predictions_original_scale[:, i], alpha=0.5)
        axes[i].plot([targets_original_scale[:, i].min(), targets_original_scale[:, i].max()],
                     [targets_original_scale[:, i].min(), targets_original_scale[:, i].max()], 'r--')
        axes[i].set_xlabel(f"Actual {name}")
        axes[i].set_ylabel(f"Predicted {name}")
        axes[i].set_title(f"Actual vs. Predicted {name}\nR2: {r2_score(targets_original_scale[:, i], predictions_original_scale[:, i]):.2f}")
        axes[i].grid(True)
    plt.tight_layout()
