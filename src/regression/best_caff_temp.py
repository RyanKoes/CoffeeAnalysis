...
def build_largebn_model(input_size: int) -> nn.Module:
	"""LargeBN-1024-512-1 architecture.

	This matches the definition used in nn_model_window_search.get_network_architectures().
	"""
...
def main():
	parser = argparse.ArgumentParser(
		description=(
			"Train caffeine model (HPLC_Caff) with LargeBN-1024-512-1 "
			"on fixed 0.4–1.4 V window using leave-one-coffee-out CV."
		)
	)
	parser.add_argument(
		"--epochs",
	...
	print(
		f"Training {target_name} model with LargeBN-1024-512-1 on "
	...
	for fold, test_coffee in enumerate(tqdm(coffees, desc="Leave-one-coffee-out folds")):
		...
		# Build model
		model = CoffeeNetBase()
		model.network = build_largebn_model(input_size)
		model.to(device)
	...
	base_name = f"caff_LargeBN-1024-512-1_V{v_start:.2f}-{v_end:.2f}"
