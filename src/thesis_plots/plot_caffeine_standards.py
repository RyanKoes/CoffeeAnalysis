...
try:
	...
	raise ImportError(
		"ThesisPlotGeneration.plot_caffeine_standards requires 'matplotlib'. "
...
def _parse_ppm_from_filename(path: Path) -> float:
	...
	fallback = re.match(r"(\d+(?:\.\d+)?)", path.stem)
	if fallback:
...
def plot_caffeine_standards(
	*, standards_dir: Path, out_path: Path, window: int = 20
	...
	ax.set_title(f"Caffeine Standards")
	ax.grid(True, alpha=0.25)

	# Cap the y-axis top at 200 (per request) while keeping the lower limit data-driven.
	if all_y:
	...
	ax.legend(title="Caffeine (ppm)", frameon=False, ncol=2)
	plt.tight_layout()
...
def _build_argparser() -> argparse.ArgumentParser:
	parser = argparse.ArgumentParser(
		description="Plot caffeine standards from ThesisPlotGeneration/caffeine_standards and save as PDF."
	)
	parser.add_argument(
		"--standards-dir",
		type=Path,
		default=Path(__file__).resolve().parent / "caffeine_standards",
		help="Directory containing *ppm.txt standards files (default: ThesisPlotGeneration/caffeine_standards)",
	)
	parser.add_argument(
		"--window",
		type=int,
	...
	parser.add_argument(
		"--out",
		type=Path,
		default=Path(__file__).resolve().parent / "plots" / "caffeine_standards.pdf",
		help="Output PDF path (default: ThesisPlotGeneration/plots/caffeine_standards.pdf)",
	)
	return parser
...
def main() -> None:
	...
	plot_caffeine_standards(standards_dir=args.standards_dir, out_path=args.out, window=args.window)
