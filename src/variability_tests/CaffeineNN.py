...
CAFFEINE_DIR = "Caffeine Standards"
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print(f"Using device: {device}")


# Define a simple neural network
class CaffeineNet(nn.Module):
    def __init__(self, input_size):
        super(CaffeineNet, self).__init__()
        ...
        self.network = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(0.1),
            nn.Linear(256, 1)  # Single output for caffeine concentration
        )
...
def train_model(model, X_train, y_train, X_val, y_val, num_epochs=1000, batch_size=32):
...
if __name__ == "__main__":
    ...
    # txt_files = sorted(CAFFEINE_DIR.glob("*.txt"))
    txt_files = sorted(Path(CAFFEINE_DIR).glob("*.txt"))
    ...
    voltammograms = []
    for f in txt_files:
        df = load_voltammogram(f)
        # Only cpature a subset of the voltammogram
        df = df[(df['Applied Voltage'] >= 1) & (df['Applied Voltage'] <= 1.35)]
        voltammograms.append(df['Detected Response'].values)

    # Convert to numpy array
    X_all = np.array(voltammograms)

    # Define caffeine concentrations (ppm)
    y_all = np.array([200] * 5 + [500] * 5 + [800] * 5)

    print(f"Data shape: {X_all.shape}")
    print(f"Caffeine concentrations: {y_all}")
    ...
    for i in range(len(X_all)):
        ...
        model = CaffeineNet(input_size).to(device)
        ...
        model = train_model(model, X_train_final, y_train_final, X_val, y_val, num_epochs=400)
    ...
    print("\n" + "=" * 80)
    print("ACTUAL VS PREDICTED CAFFEINE CONCENTRATION (ppm)")
    print("=" * 80)
    prediction_table = {
    ...
    plt.xlabel('Actual Caffeine Concentration (ppm)', fontsize=12, fontweight='bold')
    plt.ylabel('Predicted Caffeine Concentration (ppm)', fontsize=12, fontweight='bold')
    plt.title(
        f'Actual vs Predicted Caffeine Concentration\nR² = {overall_test_r2:.4f}, MAE = {overall_test_mae:.2f} ppm',
    ...
    # Save plot
    plot_path = 'caffeine_predictions.png'
    plt.savefig(plot_path, dpi=300, bbox_inches='tight')
    print(f"\nPrediction plot saved to {plot_path}")
    plt.show()

    # Save results
    results_path = 'caffeine_results.pkl'
