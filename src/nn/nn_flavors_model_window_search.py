...
if __name__ == "__main__":
    ...
    # Drop samples missing any target flavor score
    # (We print counts first because CoffeeHub exports can contain sparse labels.)
    pre_counts = df_all[FLAVOR_COLUMNS].notna().sum().sort_values()
    pre_total = len(df_all)
    pre_complete = int(df_all[FLAVOR_COLUMNS].notna().all(axis=1).sum())
    print(f"Flavor label availability (non-null counts out of {pre_total} samples):")
    print(pre_counts.to_string())
    print(f"Samples with all {len(FLAVOR_COLUMNS)} flavor labels present: {pre_complete}")

    n_before = len(df_all)
    df_all = df_all.dropna(subset=FLAVOR_COLUMNS)
    n_after = len(df_all)
    if n_after < n_before:
        print(f"Dropped {n_before - n_after} samples with missing flavor scores")

    if n_after == 0:
        raise ValueError(
            "No samples remain after dropping rows with missing flavor scores. "
            "(see counts printed above)"
        )
    ...
    for vmin, vmax in windows:
        ...
        for arch in architectures:
            ...
            for test_coffee in coffees:
                ...
                y_train = df_train[FLAVOR_COLUMNS].values
                y_test = df_test[FLAVOR_COLUMNS].values
            ...
            per_target_r2 = {}
            for idx, col in enumerate(FLAVOR_COLUMNS):
                try:
    ...
    if best_metadata is not None:
        ...
        y_all = df_all[FLAVOR_COLUMNS].values
