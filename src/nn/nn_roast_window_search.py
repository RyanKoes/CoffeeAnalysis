...
if __name__ == "__main__":
    ...
    # Dedicated roast-level prediction search (no CLI args).
    target_name = "Roast"
    ...
    roast_missing = int(df_all[target_name].isna().sum()) if target_name in df_all.columns else None
    print(f"Loaded {n_samples} samples across {n_coffees} unique coffees")
    if roast_missing is not None and roast_missing > 0:
        print(f"Warning: {roast_missing} samples have missing {target_name} values")
