...
if __name__ == "__main__":
    ...
    fig.suptitle(f'Linear Regression: Best Optimized Voltage Points {title_suffix}', fontsize=16, fontweight='bold')
    ...
    for idx, (target_name, data) in enumerate(best_models.items()):
        ...
        ax_train.scatter(results['train_actual'], results['train_pred'],
                         alpha=0.6, color='blue', edgecolors='k', s=40)

        # Perfect line
        mn, mx = results['train_actual'].min(), results['train_actual'].max()
        ax_train.plot([mn, mx], [mn, mx], 'r--', linewidth=2)

        ax_train.set_xlabel('Actual')
        ax_train.set_ylabel('Predicted')
        ax_train.set_title(f'{target_names_display[target_name]} (Train)\n'
                           f'Best V: {v_point:.2f}V | R²: {results["overall_metrics"]["train_r2"]:.3f}')
        ax_train.grid(True, alpha=0.3)

        # Test Plot (Bottom Row)
        ax_test = axes[1, idx]
        ax_test.scatter(results['test_actual'], results['test_pred'],
                        alpha=0.6, color='orange', edgecolors='k', s=40)

        mn, mx = results['test_actual'].min(), results['test_actual'].max()
        ax_test.plot([mn, mx], [mn, mx], 'r--', linewidth=2)

        ax_test.set_xlabel('Actual')
        ax_test.set_ylabel('Predicted')
        ax_test.set_title(f'{target_names_display[target_name]} (Test LOOCV)\n'
                          f'R²: {results["overall_metrics"]["test_r2"]:.3f} | MAE: {results["overall_metrics"]["test_mae"]:.3f}')
        ax_test.grid(True, alpha=0.3)

    plt.tight_layout(rect=[0, 0.03, 1, 0.95])

    filename_suffix = "_baseline_corrected" if BASELINE_CORRECTION else "_raw"
    save_path = PLOTDIR / f'regression_optimized_results{filename_suffix}.png'
