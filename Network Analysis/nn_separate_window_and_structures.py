...
if __name__ == "__main__":
    ...
    for target_name, experiments in target_configs.items():
        ...
        print(f"Voltage range: {voltage_ranges[target_name][0]} to {voltage_ranges[target_name][1]} V")
        print(f"{'=' * 60}")
        ...
        for experiment in tqdm(experiments, desc=f"{target_name} experiments"):
            ...
            print(f"Coffee names used in this experiment ({len(coffees)}):")
            for cname in coffees:
                print(f"  - {cname}")
            ...
            if 'voltages' in df_all.columns:
                ...
                print("WARNING: Voltage array not found in dataframe. Please ensure voltages are available.")
                voltages = None
            ...
            for fold, test_coffee in enumerate(coffees):
                ...
                e = {
                    ...
                    'target': target_name,
                    'voltage_range': voltage_ranges[target_name]
                }

                # Setup train data with voltage filtering
                cv_raw_list = df_train['cv_raw'].to_list()

                # Filter CV data by voltage range for this target
                if voltages is not None:
                    X_train_filtered = []
                    for cv_data in cv_raw_list:
                        filtered_cv = filter_cv_by_voltage(cv_data, voltage_ranges[target_name], voltages)
    ...
    for target_name in target_configs.keys():
        print(f"\n{'=' * 60}")
        print(f"Results for {target_name} (Voltage Range: {voltage_ranges[target_name]})")
        print(f"{'=' * 60}")
    ...
    for target_name in target_configs.keys():
        ...
        summary_data.append({
            'Target': target_name,
            'Voltage Range': f"{voltage_ranges[target_name][0]}-{voltage_ranges[target_name][1]}V",
